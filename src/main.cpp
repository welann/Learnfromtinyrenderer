...
void draw(std::vector<std::string>& Mname,float* zbuffer){
    // for(int i=0; i<Mname.size();i++){
        // const char* temp=Mname[0].c_str();
        // Model model1(temp);
        // temp=Mname[1].c_str();
        // Model model2(temp);
        ...
        { // rendering the frame buffer
            TGAImage frame(width, height, TGAImage::RGB,TGAColor(128,164,146));
...
int main(int argc, char** argv) {
    ...
    std::vector<std::string> Mname={"obj/african_head.obj","obj/african_head_eye_inner.obj"};
