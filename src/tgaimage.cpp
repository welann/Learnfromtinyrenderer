...
//用一种颜色初始化整个画布
TGAImage::TGAImage(int w, int h, int bpp, TGAColor color): data(NULL), width(w), height(h), bytespp(bpp){
    //需要先初始化data的空间才能继续运行，否则会内存溢出
    unsigned long nbytes = width*height*bytespp;
    data = new unsigned char[nbytes];
    for(int i = 1; i <w; i++){
        for(int j = 1; j < h; j++){
            memcpy(data+(i+j*width)*bytespp, color.bgra, bytespp);
        }
    }
}
